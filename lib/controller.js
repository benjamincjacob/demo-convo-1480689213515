...
module.exports = {
	...
	processMessage: function(_message, source, callback) {
		...
		/*		if (message.input.test === '/switch') {
					if (message.context.intentfdbk === 'true'){
						message.context.intentfdbk === 'false';
					}else{
						message.context.intentfdbk === 'true';
					}
				}*/
		//		message = extend({context: {}});
		if (message.input.text === 'testingxxxxxx') {
			message.context.first = 'false';
			sendMessageToConversation(message).then(function(messageResponse) {
					//debug('2. Conversation response: %s.', JSON.stringify(messageResponse, null, 2));


					// If an intent was detected, log it out to the user.
					if (messageResponse.intents.length > 0) {
						messageResponse.output.text = 'Detected intent: #' + messageResponse.intents[0].intent + ' Score: ' + messageResponse.intents[0].confidence;
					}

					debug('4. Conversation response to user: %s.', JSON.stringify(messageResponse, null, 2));
					messageResponse.context.system.dialog_stack[0] = "node_1_1475014595024";
			...
			getUser(user).then(function(dbUser) {
				...
				.then(function(messageResponse) {
					...
					.then(function(messageResponse) {
						...
						if (messageResponse.input.text === '/switch') {
							debug('6. Switching the convo mode');
							if (messageResponse.context.intentfdbk === 'false') {
								messageResponse.context.intentfdbk === 'true';
								// If an intent was detected, log it out to the user.
								if (messageResponse.intents.length > 0) {
									messageResponse.output.text = 'Detected intent: #' + messageResponse.intents[0].intent + ' Score: ' + messageResponse.intents[0].confidence;
								}
								...
								messageResponse.context.intentfdbk === 'false';
							}
							return messageResponse;
						}
